            ...
            if cv_only:
                ...
                for i, (train_fold, validate) in enumerate(fold_rs):
                    ...
                    with ut.tick_tock("round {}".format(i)):
                        ...
                        train_train = train.iloc[train_fold, :]
                        train_test = train.iloc[validate, :]
                        ...
                        # else:
                        tgt_feas = ['var_68', 'var_91', 'var_108', 'var_103']
                        tgt_feas = ['var_68', 'var_91', 'var_108', 'var_103',
                                    'var_12', 'var_148', 'var_161', 'var_71', 'var_25', 'var_43',
                                    'var_125', 'var_166', 'var_169', 'var_133', 'var_15', 'var_131',
                                    'var_23', 'var_34', 'var_93', 'var_95', 'var_42', 'var_50',
                                    'var_126', 'var_98', 'var_53', 'var_57', 'var_28', 'var_130',
                                    'var_59', 'var_6', 'var_105', 'var_197', 'var_144', 'var_156',
                                    'var_189', 'var_114', 'var_111', 'var_66', 'var_9', 'var_79',
                                    'var_181', 'var_162', 'var_195', 'var_132', 'var_192', 'var_64']

                        # tgt_feas = ['catcount3_ID_code_var_133_count']

                        for f in tqdm(tgt_feas):
                            train[f + "_tef"], test[f + "_tef"] = flb.target_encode(trn_series=train_train[f],
                                                                                    tst_series=train_test[f],
                                                                                    target=train_train['target'],
                                                                                    min_samples_leaf=30,
                                                                                    smoothing=3,
                                                                                    noise_level=0.1)
                            print " train_tef", train[f + "_tef"].head(50)
                        pdf_cols = ["{}_tef".format(i) for i in tgt_feas]

                        print predictors + cat + pdf_cols, "predictors + cat + pdf_cols"

                        X = train[predictors + cat + pdf_cols]
                        X_test = test[predictors + cat + pdf_cols]
