...
with ut.tick_tock('read_data'):
    ...
    # out_cols = list([cst.key,cst.label])
#
#     print out_cols
#
with ut.tick_tock("write data"):
    train_ori[[cst.key,cst.label]].to_csv(cst.train_prefix + key + '.csv', index=False)
    test_base[[cst.key]].to_csv(cst.test_prefix + key + '.csv', index=False)
