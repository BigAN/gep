            ...
            if cv_only:
                ...
                for i, (train_fold, validate) in enumerate(fold_rs):
                    ...
                    with ut.tick_tock("round {}".format(i)):
                        ...
                        df_feature = pd.concat([train, test])
                        df_target = train.loc[train_fold, :]
                        tgt_feas = ['catcount3_ID_code_var_{}_count'.format(i) for i in range(200)]
                        for i in tgt_feas:
                            df_feature[i + "_pdf"] = ut.calculate_pdf_difference(i, df_feature, df_target, 0.8, 0.1,False)

                        pdf_cols = ["{}_pdf".format(i) for i in tgt_feas]

                        print predictors + cat + pdf_cols, "predictors + cat + pdf_cols"
                        train = df_feature[:len(train)]
                        test = df_feature[len(train):]
