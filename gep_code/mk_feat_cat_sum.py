...
def feature_func(out, feature):
    feature['sum'] = feature.mean(axis=1)
    cat_feas = [x for x in out.columns.tolist() if "var_" in x]
    for i in cat_feas:
        weidu = [i]
        # out = flb.feat_max(out, feature, weidu, 'sum')
        # out = flb.feat_ptp(out, feature, weidu, 'sum')
        # out = flb.feat_min(out, feature, weidu, 'sum')
        # out = flb.feat_sum(out, feature, weidu, 'sum')
        out = flb.feat_mean(out, feature, weidu, 'sum')
        out = flb.feat_std(out, feature, weidu, 'sum')

        # out = flb.feat_std(out, feature, weidu, 'sum')
        # out = flb.feat_skew(out, feature, weidu, 'sum')
