...
class Model(object):
    ...
    def __init__(self, n_hash, n_cat, n_con, EMBEDDING_DIM):
        self.use_negsampling = False

        with tf.name_scope('Inputs'):
            # self.mid_his_batch_ph = tf.placeholder(tf.int32, [None, None], name='mid_his_batch_ph')
            self.cat_ph = tf.placeholder(tf.int32, [None, None], name='cat_ph')
            self.dense_ph = tf.placeholder(tf.float32, [None, n_con], name='dense_ph')
            self.target_ph = tf.placeholder(tf.float32, [None, None], name='target_ph')
            self.lr = tf.placeholder(tf.float64, [])

        # Embedding layer
        with tf.name_scope('Embedding_layer'):
            self.cat_embeddings_var = tf.get_variable("cat_embedding_var", [n_hash, EMBEDDING_DIM])
            self.cat_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, self.cat_ph)
            self.cat_batch_embedded = tf.reshape(self.cat_batch_embedded, [-1, EMBEDDING_DIM * n_cat])
            print " reshape self.cat_batch_embedded.shape", self.cat_batch_embedded.get_shape()

    def build_fcn_net(self, inp, use_dice=True):
        # fc1 = Dense(512, init='he_normal')(flatten)
        # fc1 = PReLU()(fc1)
        # # fc1 = BatchNormalization()(fc1)
        # fc1 = Dropout(0.5)(fc1)  # 99.88

        x = tf.layers.dense(inp, 200, activation=None, name='f1')
        if use_dice:
            x = dice(x, name='dice_1')
        else:
            x = prelu(x, 'prelu1')
        x = tf.layers.batch_normalization(inputs=x, name='bn1')
        x = tf.layers.dropout(inputs=x, rate=0.5, name='bn1')

        x = tf.layers.dense(x, 80, activation=None, name='f2')
        if use_dice:
            x = dice(x, name='dice_2')
        else:
            x = prelu(x, 'prelu2')
        x = tf.layers.batch_normalization(inputs=x, name='bn2')
        x = tf.layers.dropout(inputs=x, rate=0.5, name='bn2')

        dnn3 = tf.layers.dense(x, 2, activation=None, name='f3')
    ...
    def predict(self, sess, inps):
        probs = sess.run(self.y_hat, feed_dict={
            self.cat_ph: inps[0],
            self.dense_ph: inps[1],
        })
        return probs
...
class Model_DNN(Model):
    def __init__(self, n_hash, n_cat, n_con, EMBEDDING_DIM):
        super(Model_DNN, self).__init__(n_hash, n_cat, n_con, EMBEDDING_DIM)
        print "self.cat_batch_embedded", self.cat_batch_embedded.get_shape()
        print "self.dense_ph", self.dense_ph.get_shape()

        # inp = tf.concat([self.uid_batch_embedded, self.item_eb, self.item_his_eb_sum], 1)
        inp = tf.concat([self.cat_batch_embedded, self.dense_ph], 1)
        print "self.inp", inp.get_shape()
...
class Model_DIN(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False):
        ...
        #     -1)
        inp = tf.concat(
            [self.uid_batch_embedded, self.item_eb, att_fea],
            -1)
