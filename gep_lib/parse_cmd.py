...
import argparse
from gep_lib.utils import tick_tock
import glob
import itertools
...
def init_arguments_feature():
    ...
    parser.add_argument('-t', '--test_nrows', type=is_none, dest="test_nrows", default=100,
                        help="map key(rider or rider_poi)")
...
def init_arguments():
    ...
    parser.add_argument('--md5', type=str, dest="md5", default=-1,
                        help="md5 for exp")
        ...
        with tick_tock("start add feature"):
            for n, af in enumerate(add_feas_files):
                if len(af) > 1:
                    ...
                    train_to_append = pd.read_csv(train_file, usecols=be_added)
                    test_to_append = pd.read_csv(test_file, usecols=be_added, nrows=nrows)
